...
module.exports.injectPluginFactory = function (factory, name) {
    var pluginLoadTimeout;
    var depsLoadTimeout;

    function mergeFactoryConf(userConf, factory) {
        var conf;
        // 合并默认配置
        if (factory.defaultConf) {
            conf = _.extend(_.cloneDeep(factory.defaultConf), _.cloneDeep(userConf) || {});
        }
        else {
            conf = _.cloneDeep(userConf) || {};
        }
        return conf;
    }

    function runFactory(factory, cb) {
        // 注册这个pluginFactory方便以后调用
        yog.pluginFactories[name] = function (app, conf, cb) {
            conf = conf ?
                mergeFactoryConf(conf, factory) :
                mergeFactoryConf(yog.conf[name], factory);
            return factory(app, conf, cb);
        };
        // 初始化时第一次调用这个pluginFactory
        var start = +(new Date());
        var done = false;
        ...
        // 合并默认配置
        var conf = mergeFactoryConf(yog.conf[name], factory);
        if (conf.YOG_DISABLE) {
...
function readdirWithEnvSync(path, prefer) {
    // prefer由yog所在的env决定
    // 比如在'dev'下prefer就为'.dev'
    var loaded = {};
    var files = [];
    readdirSync(path).forEach(function (subPath) {
        if (loaded[p.normalize(subPath)]) {
            // 先读到了带有其他环境标识的basename相同，subfix相同的文件
            // 当时通过getPreferFile已经读到了本文件
            return;
        }
        var ext = p.extname(subPath);
        var filePath = null;
        ...
        if (basename[0] === '.' || loaded[confID]) {
            // basename'.'开头说明是隐藏文件，不读
            // basename相同，subfix相同，环境标识不同，根据prefer已加载了那个优先级最高的
            return;
        }
